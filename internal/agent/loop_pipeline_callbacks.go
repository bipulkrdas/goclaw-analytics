...
func (l *Loop) pipelineCallbacks(req *RunRequest, bridgeRS *runState) pipelineCallbackSet {
	...
	return pipelineCallbackSet{
		...
		injectSessionState: l.makeInjectSessionState(req),
		...
		detectSessionState: l.makeDetectSessionState(req),
...
type pipelineCallbackSet struct {
	...
	injectSessionState func(ctx context.Context, input *pipeline.RunInput) (string, error)
...
// makeInjectSessionState builds the callback that injects dynamic session state
// (e.g. IoT analytics data summary) into the system prompt at the start of each run.
// Reads session metadata: if "iot_data_ready" is "true", appends the stored
// metadata summary so the LLM knows data.csv is available for Phase 2 analytics.
// If data is not ready but device_id is set, injects device context for Phase 1.
func (l *Loop) makeInjectSessionState(req *RunRequest) func(ctx context.Context, input *pipeline.RunInput) (string, error) {
	return func(ctx context.Context, input *pipeline.RunInput) (string, error) {
		if input.SessionKey == "" {
			return "", nil
		}
		meta := l.sessions.GetSessionMetadata(ctx, input.SessionKey)
		if meta == nil {
			return "", nil
		}
		deviceID := meta["iot_device_id"]
		if deviceID == "" {
			return "", nil
		}

		if meta["iot_data_ready"] == "true" {
			summary := meta["iot_metadata_summary"]
			if summary == "" {
				return "", nil
			}
			return "## Session State\ndata_ready: true\ndevice_id: " + deviceID + "\n\n## Data Summary\n" + summary + "\n\nData file is available at /output/data.csv in the sandbox. Use it for analytics queries.\nDo NOT re-fetch from the API unless the user explicitly changes the time range.", nil
		}

		// Phase 1: data not ready — tell the LLM which device to target
		return "## Session State\ndata_ready: false\ndevice_id: " + deviceID + "\n\nOn the first user message, use the iot-data-engineering skill to prepare analytics data for this device.\nExtract start_time and end_time from the user's message. Default: last 30 days if not specified.", nil
	}
}

// makeDetectSessionState builds the callback that checks if the IoT pipeline
// produced metadata.json in the workspace after tool execution. If found,
// reads the file and stores/updates the summary in session metadata for
// injection on subsequent turns. Re-reads every run to handle re-triggers
// (user requests new time range → LLM overwrites metadata.json).
func (l *Loop) makeDetectSessionState(req *RunRequest) func(ctx context.Context, sessionKey string) {
	return func(ctx context.Context, sessionKey string) {
		if sessionKey == "" || l.workspace == "" {
			return
		}
		// No device context — not an IoT session
		meta := l.sessions.GetSessionMetadata(ctx, sessionKey)
		if meta == nil || meta["iot_device_id"] == "" {
			return
		}
		// Check if metadata.json exists in the workspace output dir
		metaPath := filepath.Join(l.workspace, "output", "metadata.json")
		content, err := os.ReadFile(metaPath)
		if err != nil {
			return // file doesn't exist yet — Phase 1 hasn't completed
		}
		...
		if meta["iot_data_ready"] == "true" && meta["iot_metadata_summary"] == summary {
			return
		}
		l.sessions.SetSessionMetadata(ctx, sessionKey, map[string]string{
			"iot_data_ready":       "true",
			"iot_metadata_summary": summary,
		})
	}
}
