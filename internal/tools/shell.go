...
func (t *ExecTool) Execute(ctx context.Context, args map[string]any) *Result {
	...
	for _, pattern := range allPatterns {
		if pattern.MatchString(normalizedCommand) {
			...
			// Auto-approve when sandbox is active — container isolation makes it safe.
			if matchesAny(normalizedCommand, pkgInstallPatterns) {
				sandboxKey := ToolSandboxKeyFromCtx(ctx)
				if t.sandboxMgr != nil && sandboxKey != "" {
					// Sandbox is active — auto-approve, package install is isolated.
					slog.Info("exec: package install auto-approved (sandbox)", "command", truncateCmd(command, 100), "agent", t.agentID)
					continue
				}
