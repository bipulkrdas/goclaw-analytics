...
func (t *ListFilesTool) Execute(ctx context.Context, args map[string]any) *Result {
	...
	// Skip sandbox for host-only paths (skills-store, etc.)
...
// isHostOnlyPath returns true if the path is within an allowed prefix (skills-store, etc.)
// that only exists on the host, not inside the sandbox container.
func (t *ListFilesTool) isHostOnlyPath(filePath string) bool {
