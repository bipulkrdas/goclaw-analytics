...
func (t *ReadFileTool) Execute(ctx context.Context, args map[string]any) *Result {
	...
	// Skip sandbox for host-only paths (skills-store, CLI workspaces) — these
	// exist only on the host filesystem and aren't mounted into the container.
...
// isHostOnlyPath returns true if the given path is within an allowed prefix
// (e.g. skills-store) that only exists on the host, not inside the sandbox container.
func (t *ReadFileTool) isHostOnlyPath(filePath string) bool {
	if !filepath.IsAbs(filePath) {
		return false
	}
	cleaned := filepath.Clean(filePath)
	for _, prefix := range t.allowedPrefixes {
		if prefix != "" && strings.HasPrefix(cleaned, filepath.Clean(prefix)) {
			return true
		}
	}
	return false
