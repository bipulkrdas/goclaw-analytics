...
func (h *FilesHandler) handleServe(w http.ResponseWriter, r *http.Request) {
	...
	// Block access to sensitive system directories (exempt configured workspace/dataDir)
	if hasDeniedFilePrefix(absPath) && !h.isConfiguredRoot(absPath) {
...
// isConfiguredRoot returns true if the path falls under the handler's configured
// workspace or dataDir. These are trusted operator-configured roots that should
// be exempt from the deny-prefix blanket block.
func (h *FilesHandler) isConfiguredRoot(path string) bool {
	cleaned := filepath.Clean(path)
	if ws := configuredFileRoot(h.workspace); ws != "" && pathWithinDir(cleaned, ws) {
		return true
	}
	if dd := configuredFileRoot(h.dataDir); dd != "" && pathWithinDir(cleaned, dd) {
		return true
	}
	return false
}
...
func (h *FilesHandler) openValidatedFile(r *http.Request, absPath string, signed bool) (*os.File, string, os.FileInfo, bool) {
	...
	if hasDeniedFilePrefix(realPath) && !h.isConfiguredRoot(realPath) {
		slog.Warn("security.files_realpath_denied", "path", absPath, "resolved", realPath, "handler_workspace", h.workspace, "handler_datadir", h.dataDir)
		_ = file.Close()
		return nil, "", nil, false
	}
